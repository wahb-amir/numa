...
const STATUS_STYLES: Record<StatusLevel, { dot: string; text: string; bg: string; label: string }> = {
  positive: {
...
export function StatusDot({ status, className }: { status: StatusLevel; className?: string }) {
  return (
    <span
      aria-hidden="true"
      className={cn("inline-block h-2 w-2 rounded-full", STATUS_STYLES[status].dot, className)}
    />
  );
}

export function StatusChip({ status, children }: { status: StatusLevel; children?: React.ReactNode }) {
