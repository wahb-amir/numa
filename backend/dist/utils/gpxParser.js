...
exports.parseGpxDetailed = exports.parseGpx = exports.GpxParserError = void 0;
 ...
 */
class GpxParserError extends Error {
    phase;
...
exports.GpxParserError = GpxParserError;
 ...
 */
const guessActivityType = (avgSpeedKmh) => {
    if (avgSpeedKmh >= AVG_CYCLING_SPEED_KMH)
        return "cycling";
    if (avgSpeedKmh >= AVG_RUNNING_SPEED_KMH / 1.25)
        return "running";
    return "other";
};
/**
...
const normalizeActivityType = (raw) => {
    ...
    if (/(bike|biking|cycling|ride|spin|cyclocross)/.test(v))
        return "cycling";
 ...
 */
const computeFingerprint = (userId, startTimeIso, durationSeconds, activityType, distance) => (0, crypto_1.createHash)("sha1")
    .update([
    userId,
    startTimeIso,
...
const parseGpx = async (buffer, userId, uploadId) => {
    const result = await (0, exports.parseGpxDetailed)(buffer, userId, uploadId);
 ...
 */
const parseGpxDetailed = async (buffer, userId, uploadId) => {
    ...
    if (validation !== true) {
        ...
        const detail = validation.err?.msg ?? "unknown";
        throw new GpxParserError("INVALID_XML", `GPX file is not valid XML: ${detail}`);
    }
    ...
    if (!rootTag.toLowerCase().includes("gpx")) {
        throw new GpxParserError("NOT_GPX", `File does not look like a GPX document (root tag: <${rootTag}>)`);
    }
    const gpxObj = gpx;
    const trkRaw = gpxObj.trk;
    if (trkRaw === undefined) {
        throw new GpxParserError("NO_TRACKS", "GPX file contains no <trk> elements");
    }
    const tracks = Array.isArray(trkRaw) ? trkRaw : [trkRaw];
    const workouts = [];
    const parseErrors = [];
    let totalPoints = 0;
    // Fallback start time taken from <metadata>/<time> if individual tracks
    // lack timestamps.
    const metadataTime = toDate(gpxObj.metadata?.time);
    for (let trackIndex = 0; trackIndex < tracks.length; trackIndex++) {
        ...
        const avgSpeed = distance !== undefined && durationSeconds > 0
        ...
        const typeRaw = Array.isArray(trackObj.type)
            ? trackObj.type[0]
            : trackObj.type;
        const typeText = typeof typeRaw === "object" && typeRaw !== null
            ? typeRaw["#text"]
            : typeRaw;
        const activityType = normalizeActivityType(typeof typeText === "string" ? typeText : undefined) ||
        ...
        if (elevationGain > 0)
            metrics.elevation_gain = elevationGain;
        if (avgHr !== undefined)
            metrics.avg_heart_rate = avgHr;
        if (maxHr !== undefined)
            metrics.max_heart_rate = maxHr;
        workouts.push({
            user_id: userId,
            ...
            metrics,
            fingerprint: computeFingerprint(userId, startTime.toISOString(), durationSeconds, activityType, distance),
            status: "valid",
        });
    }
    if (workouts.length === 0) {
        if (parseErrors.length === 0) {
            throw new GpxParserError("INSUFFICIENT_POINTS", "GPX file contained no usable tracks");
        }
        ...
        throw new GpxParserError("INSUFFICIENT_POINTS", `All GPX tracks were rejected: ${parseErrors.map((e) => e.reason).join("; ")}`);
    }
    if (parseErrors.length > 0) {
        logger_1.logger.warn(`GPX upload ${uploadId}: ${parseErrors.length} of ${tracks.length} track(s) rejected`);
    }
    return {
        workouts,
        parseErrors,
...
exports.parseGpxDetailed = parseGpxDetailed;
