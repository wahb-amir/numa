...
exports.parseCsvDetailed = exports.parseCsv = exports.CsvParserError = void 0;
...
exports.CsvParserError = CsvParserError;
/**
 ...
 */
const normalizeHeader = (raw) => raw.trim().toLowerCase().replace(/[_\-]+/g, " ").replace(/\s+/g, " ");
/**
 ...
 */
const distanceUnitFromOriginalHeader = (originalHeader) => {
    const h = originalHeader.toLowerCase();
    if (/\bmiles?\b|\bmi\b/.test(h))
        return "mi";
    if (/\bmeters?\b|\bm\b/.test(h))
        return "m";
    return "km";
};
const durationUnitFromOriginalHeader = (originalHeader) => {
    const h = originalHeader.toLowerCase();
    if (/\bminutes?\b|\bmin\b/.test(h))
        return "min";
    if (/\bhours?\b|\bhrs?\b|\bhr\b/.test(h))
        return "hr";
    return "s";
};
/**
 ...
 * column header.
 */
const toDurationSeconds = (raw, unit) => {
...
const parseCsv = async (buffer, userId, uploadId) => {
    const result = await (0, exports.parseCsvDetailed)(buffer, userId, uploadId);
...
const parseCsvDetailed = async (buffer, userId, uploadId) => {
    ...
    if (!headerRow || headerRow.length === 0) {
        throw new CsvParserError("MISSING_HEADER", "CSV file does not contain a header row");
    }
    ...
    if (headerToField.size === 0) {
        throw new CsvParserError("INVALID_HEADER", `CSV header row does not contain any recognized columns. Saw: ${headerRow.join(", ")}`);
    }
    if (unmappedColumns.length > 0) {
        logger_1.logger.warn(`CSV upload ${uploadId}: ${unmappedColumns.length} unmapped column(s): ${unmappedColumns.join(", ")}`);
    }
    // Now re-parse the full file as a stream of objects.
    const rowParser = (0, csv_parse_1.parse)({
        columns: (header) => header.map((h) => h.trim()),
        trim: true,
        skip_empty_lines: true,
        relax_quotes: true,
    ...
    rowParser.end();
    for await (const raw of rowParser) {
        totalRows++;
        ...
        const durationOriginalHeader = [...headerToField.entries()].find(([, field]) => field === "duration_seconds")?.[0];
        ...
        if (extracted.distance !== undefined) {
            const distanceOriginalHeader = [...headerToField.entries()].find(([, field]) => field === "distance")?.[0];
    ...
    if (workouts.length === 0 && parseErrors.length === 0) {
        throw new CsvParserError("NO_DATA_ROWS", "CSV file has a header but no data rows");
    }
    if (parseErrors.length > 0) {
        logger_1.logger.warn(`CSV upload ${uploadId}: ${parseErrors.length} of ${totalRows} row(s) rejected`);
    }
    return {
        workouts,
        parseErrors,
...
exports.parseCsvDetailed = parseCsvDetailed;
