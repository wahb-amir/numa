    ...
    try {
        ...
        for (const workout of parsedWorkouts) {
            const { error: insertError } = await supabase_1.supabase
                .from('workouts')
                .insert(workout);
            if (insertError && insertError.code !== '23505') { // Ignore unique violations
        ...
        const activityTypes = [...new Set(parsedWorkouts.map((w) => w.activity_type))];
        for (const type of activityTypes) {
            await queues_1.baselineQueue.add('computeBaselines', { userId, activityType: type });
        }
    }
    catch (error) {
