...
const envSchema = z
  .object({
    ...
    FRONTEND_URL: z.string().min(1, 'FRONTEND_URL cannot be empty').default('http://localhost:3000'),
  ...
  .refine(
    (data) => data.SUPABASE_PUBLISHABLE_KEY.length > 0 || data.SUPABASE_SECRET_KEY.length > 0,
    {
      message:
