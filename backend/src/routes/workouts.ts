...
workoutRouter.post('/', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    await baselineQueue.add('computeBaselines', { userId, activityType: activity_type });
...
workoutRouter.get('/:id', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user!.id;
    const { id } = req.params;
...
workoutRouter.post('/:id/reflection', requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user!.id;
